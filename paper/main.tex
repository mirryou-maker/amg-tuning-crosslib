...
\caption{Overview. We sweep 88 internal configurations of \amgcl{} over 150
(later 300) SuiteSparse matrices (1--2), measure the per-matrix oracle speedup
of the best configuration over the default (3), and train a predictor on cheap
matrix features to pick a good configuration under a leave-one-group-out
protocol (4--6). We then repeat the sweep on \hypre{} BoomerAMG to test whether
the findings generalise (7): tunability replicates but its magnitude is
default-driven and does not transfer between libraries, whereas intrinsic
solvability does.}
\label{fig:overview}
\end{figure}
