...
\title{\bfseries Tuning Internal Parameters of Algebraic Multigrid:
...
\end{enumerate}
Figure~\ref{fig:overview} summarises the study end to end.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{concept_diagram.png}
\caption{Overview of the study, which asks three questions about tuning the
internal parameters of AMG: how much is there to gain, can a good configuration
be predicted, and does it generalise? We sweep 88 internal configurations of
\amgcl{} over 150 (later 300) SuiteSparse matrices (1--2), measure the
per-matrix oracle speedup of the best configuration over the default (3), and
train a predictor on cheap matrix features to pick a good configuration under a
leave-one-group-out protocol (4--6). We then repeat the sweep on \hypre{}
BoomerAMG to test whether the findings generalise (7): tunability replicates but
its magnitude is default-driven and does not transfer between libraries, whereas
intrinsic solvability does.}
\label{fig:overview}
\end{figure}
...
row-nonzero mean/spread/extremes, relative bandwidth, pattern symmetry,
diagonal-presence fraction), Tier~1 (cheap value-based: diagonal-dominance
fraction, value asymmetry $\lVert A-A^{\top}\rVert_F/\lVert A\rVert_F$, a
Gershgorin spectral-radius bound, and a dominant-eigenvalue estimate from eight
power-iteration steps on $|A|$), and Tier~2 (a symmetric-part
condition-number estimate from a few Lanczos-type steps). Every feature is
scale-normalised across the training set.

Each solve becomes one training example whose input is the matrix feature
vector concatenated with a one-hot encoding of the configuration (coarsening
family, relaxation type, and threshold, plus the fill level for ILU($k$)). Two
gradient-boosted-tree models~\cite{friedman2001,scikit} are fit (each 120
trees, depth 3): a classifier $S$ for $\Pr[\text{solve}]$ over all runs, and a
regressor $R$ for $\log t_{\text{total}}$ over the converged runs only. For a
test matrix we score all configurations and pick the one of smallest predicted
time among those with $S>0.5$, falling back to the highest $S$ if none clears
the threshold (Algorithm~\ref{alg:predict}). We report, per matrix, whether the
picked configuration actually converged and the fraction of the oracle
log-speedup it captured. The oracle itself is computed only over configurations
that converge with the non-degenerate right-hand side.

\textbf{All predictor numbers use leave-one-group-out cross-validation}: every
matrix from a held-out SuiteSparse group is excluded from training, so no
sibling leaks across the split. The cross-library transfer experiments
(Section~\ref{sec:crosslib}) use the same features but at the matrix level ---
predicting ``does any configuration solve this matrix?'' and the per-matrix
oracle magnitude --- so a model trained on one library can be applied to the
other despite their disjoint configuration spaces.
...
(Nos.\ RS-2025-25463492 and RS-2026-25472340) and the Strategic Research
Program under the DGIST R\&D
...
extractor, the predictor, and all figure-generation scripts are released at
\url{https://github.com/mirryou-maker/amg-tuning-crosslib}. A single command,
\texttt{python reproduce.py}, regenerates every figure and headline number from
the released data.

For practitioners, the repository also ships the predictor as a ready-to-use
tool: \texttt{python tools/recommend.py your\_matrix.mtx} extracts the cheap
features, trains the predictor on all released sweep data, and prints a ranked
short-list of configurations --- for either \amgcl{} or \hypre{} --- with
parameter strings that paste directly into the respective solver's runtime
interface. Consistent with the leave-one-group-out results, the top suggestion
is a heuristic (it solves $\sim\!83$--$89\%$ of unseen matrices and captures a
median $\sim\!43$--$49\%$ of the oracle), so the intended use is to try the top
few candidates and keep the fastest.
